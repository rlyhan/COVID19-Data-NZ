...
import { months, events } from './general-data'
...
// Returns an array of date objects between start and end dates, inclusive 
export function getDateList(startDate, endDate) {
  for (var dateList = [], d = new Date(startDate); d <= new Date(endDate); d.setDate(d.getDate() + 1)) {
...
// Find and return event that matches given date
export function findEvent(aDate) {
  return events.find(event => convertDateToString(event.date, 'simple') === convertDateToString(aDate, 'simple'))
}
